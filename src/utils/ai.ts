// AI utility functions for task enhancement
    ...
    export function estimateTaskDuration(title: string, description: string): string {
      const shortTasks = ['مراجعة', 'اتصال', 'رد', 'تحديث', 'تعديل', 'إرسال', 'تصفح', 'قراءة سريعة'];
      const mediumTasks = ['اجتماع', 'تقرير', 'تحضير', 'تخطيط', 'تنظيم', 'بحث', 'مذاكرة', 'تجميع'];
      const longTasks = ['مشروع', 'تطوير', 'دراسة', 'تحليل', 'تصميم', 'إنشاء', 'كتابة', 'برمجة'];
      ...
      // If no specific keywords, estimate based on length and complexity
      if (title.length > 50 || description.length > 120) {
         return 'ساعتين إلى ثلاث ساعات';
      }
      if (title.split(' ').length > 7 || description.split(' ').length > 20) {
        return 'ساعة ونصف';
      }
      return 'ساعة واحدة';
    }
