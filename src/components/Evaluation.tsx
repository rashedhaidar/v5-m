import React, { useState, useRef, useContext } from 'react';
    ...
    import { CheckCircle, XCircle, Brain, TrendingUp, Award, Calendar, Download, Upload, Check, X, AlertTriangle, FileDown, FileUp } from 'lucide-react';
        ...
        <div className="p-6 bg-gradient-to-br from-purple-900/90 to-pink-900/90 rounded-lg shadow-lg" dir="rtl">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-black/20 p-4 rounded-lg">
                <h3 className="text-white font-medium mb-2">الكتابة الحرة</h3>
                {DAYS.map((_, dayIndex) => {
                  const freeWriting = localStorage.getItem(`freeWriting-${weekNumber}-${year}-${dayIndex}`);
                  return freeWriting && (
                    <div key={dayIndex} className="mb-2">
                      <h4 className="text-white/70 text-sm">{DAYS[dayIndex]}</h4>
                      <p className="text-white">{freeWriting}</p>
                    </div>
                  );
                })}
              </div>
              <div className="bg-black/20 p-4 rounded-lg">
                <h3 className="text-white font-medium mb-2">القرارات</h3>
                {DAYS.map((_, dayIndex) => {
                  const decisions = localStorage.getItem(`decisions-${weekNumber}-${year}-${dayIndex}`);
                  return decisions && (
                    <div key={dayIndex} className="mb-2">
                      <h4 className="text-white/70 text-sm">{DAYS[dayIndex]}</h4>
                      <p className="text-white">{decisions}</p>
                    </div>
                  );
                })}
              </div>
